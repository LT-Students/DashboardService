...
public class CreateTaskRequestValidator : AbstractValidator<CreateTaskRequest>, ICreateTaskRequestValidator
{
    ...
    )
  {
    ...
    RuleFor(request => request.Name)
      .NotNull().WithMessage("Task name is empty.")
      .MaximumLength(50).WithMessage("Task name is too long.");

    RuleFor(request => request.Content)
      .NotNull().WithMessage("Task content is empty.");
    ...
    When(request => request.TaskTypeId.HasValue, () =>
    {
      RuleFor(request => request.TaskTypeId)
        ...
        .MustAsync(async (taskTypeId, _) => await taskTypeRepository.ExistAsync(taskTypeId.Value))
    ...
    When(request => request.PriorityId.HasValue, () =>
    {
      RuleFor(request => request.PriorityId)
        ...
        .MustAsync(async (priorityId, _) => await priorityRepository.ExistAsync(priorityId.Value))
