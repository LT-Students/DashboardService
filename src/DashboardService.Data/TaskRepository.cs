...
public class TaskRepository : ITaskRepository
{
  ...
  {
    IQueryable<DbTask> dbTasks = _provider.Tasks.Where(t=>t.GroupId == filter.GroupId).AsQueryable();
  ...
  {
    return await IncludeRelatedEntities(filter, _provider.Tasks.AsQueryable()).FirstOrDefaultAsync(t=>t.Id == id, cancellationToken);
  }

  public async Task<bool> EditAsync(Guid id, JsonPatchDocument<DbTask> request)
  {
